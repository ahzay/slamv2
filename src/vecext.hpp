...
template <typename T> class vecext : public std::vector<T> {
public:
...
template <typename T> vecext<T> pow(const vecext<T> v, const T s) {
  vecext<T> d(v);
  for (T &j : d)
    j = pow(j, s);
  return d;
...
template <typename T> vecext<T> operator+(T s, vecext<T> v) {
  for (T &d : v)
    d += s;
  return v;
}
...
// linspace
template <typename T> vecext<T> linspace(T a, T b, size_t N) {
...
// covariance
template <typename T> T cov(vecext<T> x, vecext<T> y) {
